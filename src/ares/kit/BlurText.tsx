...
import { aresFill, ease, FONT_FAMILY } from '../ares-tokens';

/**
 * BlurText — the inspo's kinetic typography. Words BUILD IN one at a time (each
 * blurs+rises in, staggered), then the whole line blurs out on the swap. The
 * line is white/ink; the ONE accent word flows with the Ares gradient (OnAir
 * effect) — `accentMode` picks the bloom (white-anchored) or lavender
 * (blue-anchored) variant. Optional blinking cursor. One line, centered at `top`.
 */
    ...
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
      ...
      >
        {words.map((w, i) => {
          ...
          const em: React.CSSProperties = isEm ? { ...aresFill(frame, accentMode === 'bloom'), fontStyle: 'italic' } : { color };
          return (
