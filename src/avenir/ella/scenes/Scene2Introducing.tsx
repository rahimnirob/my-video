...
/**
 * Scene 2 — INTRODUCING (f198–318), on the mint field. "Introducing" slides in,
 * then the Ella mark + "Ella" wordmark build in BESIDE it on ONE line, with a
 * small "Validate before you build." positioning sub. Holds, then blurs out.
    ...
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ opacity: op, filter: `blur(${outP * 8}px)`, transform: `translateY(${-outP * 12}px)`, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 26 }}>
          ...
          >
            <Img src={staticFile('Logo-removebg-preview.png')} style={{ height: 110, filter: 'brightness(0)' }} />
          </div>

          {/* Staggered "Ella" letters */}
          <span style={{ display: 'flex', gap: 2 }}>
            {letters.map((char, index) => {
              const letterStart = 218 + index * 6;
        ...
        <div style={{ opacity: subP, fontFamily: FONT_FAMILY, fontWeight: 500, fontSize: 30, color: EL.gray, letterSpacing: '0.04em' }}>
          <span style={{ ...ellaFill(f, false), fontStyle: 'italic' }}>Validate</span> before you build.
        </div>
      </div>
    </AbsoluteFill>
  );
