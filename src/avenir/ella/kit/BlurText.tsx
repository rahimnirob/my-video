...
import { ellaFill, ease, FONT_FAMILY } from '../palette';

/**
 * BlurText — kinetic typography for Ella. Words BUILD IN one at a time (each
 * blurs+rises in, staggered), then the whole line blurs out on the swap. The
 * line is white/ink; the ONE accent word flows with the Ella gradient —
 * `accentMode` picks the bloom (white-anchored) or field (green-anchored)
 * variant. Forked from Ares BlurText (aresFill → ellaFill).
 */
    ...
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
      ...
      >
        {words.map((w, i) => {
          ...
          const em: React.CSSProperties = isEm ? { ...ellaFill(frame, accentMode === 'bloom'), fontStyle: 'italic' } : { color };
          return (
