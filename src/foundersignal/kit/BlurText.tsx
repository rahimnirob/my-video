...
import { fsFill, ease, FONT_FAMILY } from '../foundersignal-tokens';

/**
 * BlurText — kinetic typography for FounderSignal. Words BUILD IN one at a time (each
 * blurs+rises in, staggered), then the whole line blurs out on the swap. The
 * line is white/ink; the ONE accent word flows with the FounderSignal gradient —
 * `accentMode` picks the bloom (white-anchored) or field (violet-anchored)
 * variant. Adapted from Ella BlurText (ellaFill → fsFill).
 */
    ...
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
      ...
      >
        {words.map((w, i) => {
          ...
          const em: React.CSSProperties = isEm ? { ...fsFill(frame, accentMode === 'bloom'), fontStyle: 'italic' } : { color };
          return (
