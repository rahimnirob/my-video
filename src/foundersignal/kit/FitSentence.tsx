...
/**
 * FitSentence — the avenir "FitToFrame" effect, re-toned for FounderSignal: the first
 * word lands BIG and centered; as each next word slides in and joins the line,
 * the whole line scales DOWN to keep fitting the frame ("the sentence tries to
 * fit"). Reads as a zoom-out. The line is white; the one accent word flows with
 * the FounderSignal gradient. Adapted from Ella FitSentence (ellaFill → fsFill).
 */
    ...
    >
      <div style={{ display: 'flex', alignItems: 'baseline', whiteSpace: 'nowrap', fontFamily: FONT_FAMILY, fontWeight: weight, fontSize, letterSpacing: '-0.02em', gap: `${gapPx}px` }}>
        {prog.map((d, i) => {
          ...
          const em: React.CSSProperties = isEm ? { ...fsFill(frame, onBloom), fontStyle: 'italic' } : { color };
          return (
